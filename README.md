...
Convergence is a network monitoring platform that automatically discovers devices from Nautobot and collects telemetry via SNMP. Built on OpenTelemetry Collector, VictoriaMetrics, and Grafana, it provides rich device metadata enrichment and pre-built dashboards for network visibility.
...
- **SNMP Monitoring**: Interface traffic, errors, and system metrics with real interface names
...
**Current Status**: ✅ **Operational** - Monitoring 2 Cisco switches with full Nautobot enrichment
...
3. **Discover devices from Nautobot:**
   ```bash
   # List devices
...
4. **Update OTEL configuration:**
   ...
   - Restart OTEL Collector: `docker restart convergence-otel-collector`
